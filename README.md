...
- Для того, чтобы решить это неарвенство необходимо рассмотреть все варианты того, какими могут быть a и b.. 
...
```mermaid
    graph TD
        ...
        W--> R{a=0 and b=0} -- yes -->U[/x любое кроме 0/] --> J([end])
        R -- no --> E{a<0} -- yes --> T{b=0} -- yes --> Y[/решений нет/] --> J([end])
        T -- no --> I{b<0} -- yes --> O{b>a} --  yes --> P[/0>=x>a/] --> J([end])
        O -- no --> A[/0>=x>b/] --> J([end])
        I -- no --> S[/0<=x or x<b/] --> J([end])
        E -- no --> D{a=0} -- yes --> F[/b>x or x>b/] --> J([end])
        D -- no --> H{b>0} -- yes --> K{b>a} -- yes --> L[/x<=0 or x>b/] --> J([end])
        K-- no --> Z[/x<=0 or x>b/] --> J([end])
      H -- no --> X{b=0} -- yes --> C[/b>x or x>b/] --> J([end])
      X -- no --> V[/b>x or x>0/] --> J([end])
    ``` 
```
